      ...
      <div className="flex items-center justify-between gap-2 mt-4">
         {otp.map((digit, index) => (
            <React.Fragment key={index}>
               <input
                  id={`otpInput${index}`}
                  className="w-16 h-14 py-[15px] px-4 border border-spanishGray text-center font-bold rounded shadow-md outline-none placeholder:text-eerieBlack focus:border-eerieBlack"
                  type="text"
                  maxLength={1}
                  value={digit}
