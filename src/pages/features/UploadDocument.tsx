    ...
    for (let index = 0; index < selectedFiles.length; index++) {
      ...
      for (let step = 1; step <= totalSimulationSteps; step++) {
        setTimeout(() => {
          ...
          const currentProgressFileSize = (progress / 100) * fileSizeInBytes;
          setCurrentFileSize(
            parseFloat((currentProgressFileSize / (1024 * 1024)).toFixed(2))
          );
  ...
  const handleSelectDocument = (doc: DocumentsPropTypes) => {
    setSelectedDocuments((prevSelected) => {
      ...
      if (prevSelected.find((selectedDoc) => selectedDoc.id === doc.id)) {
        ...
        return prevSelected.filter((selectedDoc) => selectedDoc.id !== doc.id);
      } else {
      ...
      <div className="flex flex-col gap-6">
        <div className="flex justify-between items-center">
          <div className="flex items-center gap-2">
            ...
            <span>{docs && docs.length ? `(${docs.length})` : "(0)"}</span>
          </div>
